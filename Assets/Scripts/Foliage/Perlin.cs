...
public class Perlin
{
    ...
    static public float Noise(float x, float y, float z)
    {
        ...
        return Interpolate(w,
            Interpolate(v,
            Interpolate(u, DotProduct(perm[AA], x, y, z), DotProduct(perm[AB], x - 1, y, z)),
            Interpolate(u, DotProduct(perm[BA], x, y - 1, z), DotProduct(perm[BB], x - 1, y - 1, z))),
            Interpolate(v,
            Interpolate(u, DotProduct(perm[AA + 1], x, y, z - 1), DotProduct(perm[AB + 1], x - 1, y, z - 1)),
            Interpolate(u, DotProduct(perm[BA + 1], x, y - 1, z - 1), DotProduct(perm[BB + 1], x - 1, y - 1, z - 1))));
    }
    ...
    static public void SetSeed(string seed)
    {
        ...
        {
            perm[i] = possibleVectors[permArr[i]];
        }
