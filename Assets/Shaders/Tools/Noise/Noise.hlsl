...
float3 randomVector(float3 UV, float offset);
...
{
    // Function declarations
    void Evaluate(float3 pos, float angleOffset, out float Out, out float Cells);
    void Evaluate(float2 pos, float angleOffset, out float Out, out float Cells);
    
    // Evaluate worley noise
    void Evaluate(float3 pos, float angleOffset, out float Out, out float Cells)
    {
        float3 g = floor(pos);
        float3 f = frac(pos);
        float t = 8.0;
        float4 res = float4(t, 0, 0, 0);
        ...
        for (int z = -1; z <= 1; z++)
        {
            for (int y = -1; y <= 1; y++)
            {
                for (int x = -1; x <= 1; x++)
                {
                    ...
                    float3 offset = randomVector(lattice + g, angleOffset);
                    float d = distance(lattice + offset, f);
                    
                    if (d < res.x)
                    {
                        res = float4(d, offset.x, offset.y, offset.z);
                        Out = res.x;
                        Cells = res.y;
                    }
                }
...
// Evaluate at point UV using Scale
void EvaluateWorley_float(float3 UV, float AngleOffset, float CellDensity, out float Out, out float Cells)
{
    worley.Evaluate(UV * CellDensity, AngleOffset, Out, Cells);
}

// Evaluate at point UV using Scale
void EvaluateWorley_float(float2 UV, float AngleOffset, float CellDensity, out float Out, out float Cells)
{
    worley.Evaluate(UV * CellDensity, AngleOffset, Out, Cells);
}
...
float3 randomVector(float3 UV, float offset)
{
    float3x3 m = float3x3(52.61, 24.96, 66.04, 4.22, 70.89, 26.07, 46.86, 40.52, 21.76);
    UV = frac(sin(mul(UV, m)));
    return float3(sin(UV.z * +offset) * 0.5 + 0.5, cos(UV.x * offset) * 0.5 + 0.5, sin(UV.y * +offset) * 0.5 + 0.5);
}
